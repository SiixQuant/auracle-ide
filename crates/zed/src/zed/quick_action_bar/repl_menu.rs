...
const ZED_REPL_DOCUMENTATION: &str = "https://github.com/SiixQuant/Auracle/wiki";
...
impl QuickActionBar {
    ...
    pub fn render_repl_setup(&self, language: &str, cx: &mut Context<Self>) -> Option<AnyElement> {
        let tooltip: SharedString = SharedString::from(format!("Setup Auracle REPL for {}", language));
        Some(
            h_flex()

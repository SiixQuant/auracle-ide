    ...
    v_flex()
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some("Help improve Auracle by sending anonymous usage data".into()),
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some(
                    "Help fix Auracle by sending crash reports so we can fix critical issues fast"
                        .into(),
                ),
...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    let tooltip_description = "Auracle can only allow services like language servers, project settings, and MCP servers to run after you mark a new project as trusted.";
...
fn render_ai_section(_user_store: &Entity<UserStore>, cx: &mut App) -> impl IntoElement {
    // Auracle white-label: the hosted "Auracle Agent" (Zed-cloud) tile with its
    // "Start Free Trial" / sign-in funnel has been removed. Only BYO-key agent
    // installs (Claude/Codex/Copilot/Cursor) are offered here.

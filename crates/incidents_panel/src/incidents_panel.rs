...
impl IncidentsPanel {
    ...
    fn spawn_poll(cx: &mut Context<Self>) -> Task<()> {
        ...
        auracle_panel_common::spawn_poll(
            cx,
            POLL_EVERY,
            move || fetch_incidents(http.clone()),
...
impl Render for IncidentsPanel {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let header = panel_header("INCIDENTS", cx).child(
            Label::new("· Monitor")
        ...
        let body: AnyElement =
            match placeholder_body(&self.status, self.incidents.is_empty(), &labels) {
                        ...
                        v_flex()
                            ...
                            .child(
                                h_flex()
                                    ...
                                    .items_start()
                                    .child(
                                        div().mt_1().size_1p5().rounded_full().flex_none().bg(dot),
                                    )
                                    .child(
                                        Label::new(incident.cause.clone()).size(LabelSize::Small),
                                    )
                                    .child(div().flex_1())
